// A_MODE [DOWN, UP] straight , [RIGHT, LEFT] stop
// B_MODE [DOWN, UP] stop     , [RIGHT, LEFT] straight
// C_MODE [DOWN, UP] turn_left, [RIGHT, LEFT] stop
// D_MODE [DOWN, UP] stop     , [RIGHT, LEFT] turn_left
#include <stdio.h>
#include <stdlib.h>
#include <wiringPi.h>
#include <pthread.h>
...
#define UP_NODE "127.0.0.1"
#define DOWN_NODE "127.0.0.1"
#define LEFT_NODE "?????????"
#define RIGHT_NODE "127.0.0.1"

struct sockaddr_in addr, client_addr;
int what_node_busy[30] = { 0, };
int recv_len;
int addr_len;
int connect_on =0 ; 
...
enum B_state { NOT_BUSY, BUSY , OPEN_CONTINUE };
//enum B_state { LEFT , RIGHT , UP , DOWN };
		...
		if( digitalRead(BUTTON) == 1 )	//pressed down
		{
			if ( BUTTON == DOWN_BUTTON1 )
			{
				...
				if ( i >= BUSY_T )
				{
					...
					sendto(sock, my_arg, sizeof(send_buff), 0, (struct sockaddr *)&client_addr, sizeof(client_addr));delay(500);
				}
			}
...
int main()
{
	...
	send_buff rcv_buffer;my_node[1].now_busy = 9999;
	
	if(recvfrom(sock, &rcv_buffer, sizeof(send_buff)+1, 0, (struct sockaddr *)&client_addr, &addr_len) < 0)
	...
	pthread_create(&traffic_thd, NULL, traffic_light, (void*)DOWN_BUTTON1);
	 ...
	 while(1)
	 {
		 ...
		 what_node_busy[rcv_buffer.pin] = 1;
